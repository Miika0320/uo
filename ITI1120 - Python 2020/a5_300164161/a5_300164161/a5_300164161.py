...
def recommend(user, network):
    ...
    while i < len(network):
        if i != user:
            ...
            if len(common) > tempAmount:
                tempAmount = len(common)
                temp = i
        i+=1
    for k in network[temp][1]:
...
def maximum_num_friends(network):
    ...
    temp = 0
    for i in range(len(network)):
        if len(network[i][1])> temp:
            temp = len(network[i][1])
    return temp
...
def knows_everyone(network):
    ...
    i = 0
    flag = False
    while i < len(network) and flag == False:
...
def get_uid(network):
    ...
    while flag == False:
        ...
        if uid <= len(network):
            ...
            print("User ID invalid. Try again.")
    return uid
